  ...
  client.on('data', (msg) => {
    ...
    switch(msg.a) {
      ...
      case 'getops': {
        ...
        source.getOps(qtype, query, versions, opts, (err, data) => {
          if (err) {
          ...
          data.ops.forEach(op => op.txn = Array.from(op.txn))
          // TODO: One way or another make this consistent with how I handle it in fetch.
          write({a:'getops', ref, data})
        })

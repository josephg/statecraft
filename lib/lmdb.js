...
class Store {
  ...
  // ****** Get Ops.
  //
  // There's two strategies we use to fetch operations:
  // 1. Scan all operations in the specified range. Filter by query.
  // 2. Scan all docs in the range. Using a priority queue walk history
  // backwards to fetch all relevant operations.
  //
  // I haven't benchmarked yet, but 2. should be faster if the query is small
  // and the close of the range is close to the version head.
  //
  // 1 is O(size of requested version range)
  // 2 is O(relevant ops * size of version range start to current + cost of query)
  ...
  // Internal getOps function. Syncronous because lmdb is sync.
  _getOpsBackwards(query, from, to, supportedTypes, opts) {
    assert(opts.bestEffort || from >= this.minVersion)

    // First we run the query against the current working set to get last
    // modified versions, which we need to figure out which ops to collect.
    ...
    const addRelevant = (v, k) => {
      let docsForV = relevantOpsInTxn.get(v)
      if (docsForV == null) {
        docsForV = new Set
        relevantOpsInTxn.set(v, docsForV)
        queue.enq(v)
      }
      docsForV.add(k)
    }
    ...
    rangeutil.eachInRanges2(query, wrapLMDBCursor(snapCursor), queryRun, (k, cell, qval) => {
      ...
      // TODO: This is awful - we're decoding the entire document
      // unnecessarily! Either use a streamable decoder or write a little
      // packed binary representation here.
      const [lastMod] = msgpack.decode(cell)

      if (lastMod <= from) return false
      // Note that we track the doc even if its currently null.

      docsRead++
      addRelevant(lastMod, k)

      if (opts.limitDocs && docsRead >= opts.limitDocs) return true
      else return false
    })
    snapCursor.close()
    ...
    // Ok, we know which versions to look at now.
    while (!queue.isEmpty()) {
      const v = queue.deq()
      assert(v > from)
      ...
      // TODO: Don't actually collect the txn if the version is > requested version.
      const txn = v <= to ? new Map : null

      // entries is a list. TODO: stream decode the transaction so we don't
      // have to read everything.
      for (let i = 0; i < entries.length; i++) {
        // The entry is [k, type, lastMod, opdata, newVal].
        if (!docs.has(entries[i][0])) continue

        const [key, type, lastMod, data, newVal] = entries[i]

        if (lastMod > from) addRelevant(lastMod, key)

        if (txn != null) {
          if (supportedTypes.has(type))
            txn.set(key, {type, data})
          else
            txn.set(key, {type: 'set', data: newVal})

          if (docs.size === txn.size) break // micro optimization.
        }
      }

      if (txn != null) results.unshift({source: this.source, v, meta, txn})
    }
    txnCursor.close()

    dbTxn.commit()

    const versions = {[this.source]: [from, to]}
    if (opts.limitOps && results.length > opts.limitOps) {
      // The returned operations take us all the way to the next op in our list.
      versions[this.source][1] = results[opts.limitOps].v - 1
      results.length = opts.limitOps
    }

    // TODO: Trim output if limitOps is set.
    return {ops: results, queryRun, versions}
  }

  _getOpsForwards(query, from, to, supportedTypes, opts) {
    const dbTxn = this._env.beginTxn({readOnly:true})
  ...
  getOps(qtype, query, versions, opts, callback) {
    ...
    // Override the requested strategy based on our capabilities.
    // TODO: Is this actually a good idea? (vs just aborting saying the strategy won't work)
    const strategy = 
      //(opts.limitOps && 'forwards') ||
      (opts.limitDocs && 'backwards') ||
      opts.strategy ||
      'backwards' // TODO: Be more clever figuring out the strategy.

    //console.log('using strategy', strategy)
    const data = strategy === 'backwards'
      ? this._getOpsBackwards(query, from, to, supportedTypes, opts)
      : this._getOpsForwards(query, from, to, supportedTypes, opts)

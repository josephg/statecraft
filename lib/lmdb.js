...
class Store {
  ...
  _getOpsBackwards(query, from, to, supportedTypes, opts) {
    ...
    while (!queue.isEmpty()) {
      ...
      if (txn != null) result.unshift({source: this.source, v, meta, txn})
    }
    txnCursor.close()
    ...
    return {ops: result, queryRun, versions:{[this.source]: [from, to]}}
  }
  ...
  getOps(qtype, query, versions, opts, callback) {
    ...
    const strategy = opts.strategy ||
      ...
      'backwards' // TODO: Tweak the default based on the query.

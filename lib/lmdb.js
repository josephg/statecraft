...
class Store {
  ...
  mutate(txn, versions, options = {}, callback = doNothing) {
    ...
    const reqv = versions[this.source] != null ? versions[this.source] : this.version
    assert(typeof(reqv) === 'number')

    // TODO: Not sure if I actually want this check.
    if (reqv > this.version + 1) return callback('Requested op version in the future')
    if (reqv < this.minVersion) return callback('Operation too old for database')
    ...
    // The actual transaction version, if the transaction is accepted.
    ...
    for (let [k, op] of txn) {
      ...
      if (reqv <= lastMod) { dbTxn.abort(); return callback(Error('Write conflict - txn out of date')) }
  ...
  subscribe(qtype, initialQuery, versions, opts = {}, listener, callback) {
    ...
    sub.modify = (op, newCV, callback) => {
      ...
      rangeutil.eachInRanges(op, wrapLMDBCursor(cursor), (k, cell, val) => {
    ...
    sub._filterTxn = (txn, fn) => {
      rangeutil.eachInRanges(sub._query, txn.entries(), fn)
    }

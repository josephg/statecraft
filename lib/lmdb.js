...
class Store {
  ...
  mutate(txn, versions, options = {}, callback = doNothing) {
    ...
    for (let [k, docop] of txn) {
      ...
      txnOut.push([k, docop.type, lastMod, docop.data])
    }
  ...
  // Internal getOps functions. Syncronous because lmdb is sync.
  _getOpsCatchup(query, from, to, supportedTypes, opts) {
    ...
    while (!queue.isEmpty()) {
      ...
      for (let i = 0; i < entries.length; i++) {
        ...
        const [key, type, lastMod, data] = entries[i]
  ...
  _getOpsScan(query, from, to, supportedTypes, opts) {
    const dbTxn = this._env.beginTxn({readOnly:true})
  ...
  getOps(qtype, query, versions, opts, callback) {
    ...
    const data = strategy === 'backwards'
      ? this._getOpsCatchup(query, from, to, supportedTypes, opts)
      : this._getOpsScan(query, from, to, supportedTypes, opts)

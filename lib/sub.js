...
module.exports = (capabilities, fetch, getOps) => {
  ...
  return {
    ...
    subscribe(qtype, initialQuery, requestedVersions, opts = {}, listener) {
      ...
      // If the user requests data from previous versions we'll hold all
      // incoming ops in a list until we have all the operations -> present,
      // then send all the buffered ops out through the listener.
      //
      // This is null when we're sending data directly, or a list when we need to buffer.
      let heldOps = null

...
const resultset = require('./resultset').type
    ...
    subscribe(qtype, initialQuery, versions, opts = {}, listener) {
      ...
      const sub = { 
        ...
        versions: {
          ...
          _query: [0, 0], // client query version. Aka, the user's intended query.
          _cursor: [0, 0], // The version of workingQuery. Aka, the current active query.
        },
        ...
        _onOp(source, version, txn) {
          ...
          qops.eachTxnMatching(txn, workingQuery, (k, docop) => {
            ...
            if (supportedTypes.has(docop.type))
              ...
              update.set(k, resultset.simplifyOp(docop))

            if (this.data) {
              // This should probably be what reusltset.applyOne does.
              if (docop.type === 'rm') this.data.delete(k)
              else this.data.set(k, docop.newVal)
            }
          })
          ...
          if (update || (opts.alwaysNotify && this.versions[source])) {
            ...
            listener('txn', update || emptyMap, {[source]: vs})
          }
        },
        ...
        modify(qop) {
          ...
          if (!qops.isNoop(workOp)) {
            ...
            const cv = this.versions._cursor
            cv[0] = cv[1] = cv[1] + 1
          ...
          listener('modify', update, newVersions)
        ...
        cursorNext(opts = {}, callback) {
          ...
          fetchAtCurrent(qtype, pendingQuery, {
            ...
            for (let s in versions) {
              ...
              if (subvs == null) {
                this.versions[s] = newVersions[s] = resultvs
              } else {
            ...
            const qv = this.versions._query
            if (qv[0] !== qv[1]) {
              qv[0] = qv[1]
              newVersions._query = qv
            }

            const cv = newVersions._cursor = this.versions._cursor
            cv[1]++
            ...
            listener('next', update, newVersions)
            done()
          })
        },

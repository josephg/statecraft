...
module.exports = function wrap(store) {
  ...
  const wrapped = {
    ...
    getOps(qtype, query, versions, opts, callback) {
      if (qtype !== 'kv') return store.getOps(qtype, query, versions, opts, callback)
    ...
    subscribe(qtype, initialKV, versions, opts = {}, listener) {
      ...
      sub = {
        modify(kvop, qversion) {
          // Op is a set op. Convert to a range op.
          const skvop = rangeops.compose(
            rangeops.fromKeys(kvop.remove || [], -1),
            rangeops.fromKeys(kvop.add || [], 1)
          )

          innerSub.modify(skvop, qversion)
        },
